...
# 導入 segmentation-models-pytorch
...
class UnetAdapter(nn.Module):
    def __init__(self, exp_config):
        super().__init__()
        print("--- Initializing UNet Adapter (segmentation-models-pytorch) ---")
        ...
        # 透過 architecture_cfg 讀取 Unet 的設定
        ...
        # [修改] 如果 in_channels 不是 3，則不能使用 'imagenet' 預訓練權重
        ...
        print(f"  - UNet Config: encoder_name='{encoder_name}', encoder_weights='{encoder_weights}'")

        # 建立 smp 的 Unet 模型
        self.model = smp.Unet(
            encoder_name=encoder_name,
            encoder_weights=encoder_weights,
            in_channels=self.in_channels, # <--- [修改]
            classes=num_classes,
        )

        # 如果有提供本地權重檔案，則載入
        if base_model_path and Path(base_model_path).exists():
            print(f"  - Weight Source: Local file '{base_model_path}'")
            ...
            print(f"  - Weight Source: '{encoder_weights}' (from segmentation-models-pytorch)")

        self.model.to(self.device)
        self.names = {i: n for i, n in enumerate(self.config.get('dataset', {}).get('names', ['oil']))}
        print(f"  - Model moved to device: {self.device}")

    # (將此函式新增到 UnetAdapter class 內部)
    ...
    def train(self, data, results_path, **train_params):
        ...
        print("--- [Info] Starting UNet Training (Full Features with Plotting) ---")
        ...
        train_dataset = SegmentationDataset(train_img_dir, train_mask_dir, train_transforms, in_channels=self.in_channels) # <--- [修改] 
        train_loader = DataLoader(train_dataset, batch_size, shuffle=True, num_workers=workers, pin_memory=True)
        val_loader = DataLoader(SegmentationDataset(val_img_dir, val_mask_dir, val_transforms, in_channels=self.in_channels), batch_size, num_workers=workers, pin_memory=True) # <--- [修改]
        ...
        for epoch in range(epochs):
            ...
            total_cm_train = np.zeros((2, 2), dtype=np.int64) # <--- [新增] 初始化訓練集混淆矩陣
    ...
    def predict(self, source, imgsz, **kwargs):
        ...
        with torch.no_grad():
            self.model.eval()
            logits = self.model(image_tensor) # 這是 (imgsz, imgsz) 的 logits
            # smp 的 Unet 輸出尺寸與輸入相同，但我們仍放大回原始尺寸以確保一致性
