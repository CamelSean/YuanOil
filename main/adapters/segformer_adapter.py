    ...
    def plot(self, base_image=None):
        ...
        if self.boxes:
            ...
            for box, conf in zip(self.boxes.xyxy, self.boxes.conf):
                ...
                cv2.putText(img, f"oil {conf:.2f}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
        return img
...
class SegformerAdapter(nn.Module):
    def __init__(self, exp_config):
        super().__init__()
        print("--- Initializing SegFormer Adapter (Hugging Face Version) ---")
        self.config, self.device = exp_config, torch.device("cuda" if torch.cuda.is_available() else "cpu")
        arch_cfg, base_model_path = self.config.get('architecture_cfg', {}), self.config.get('base_model')
        num_classes = self.config.get('dataset', {}).get('nc', 1)
        hf_model_name = arch_cfg.get('hf_model_name', 'nvidia/segformer-b0-finetuned-ade-512-512')
        ...
        if base_model_path and (base_model_path.endswith('.pt') or base_model_path.endswith('.pth')):
            print(f"  - Weight Source: Local '{base_model_path}', Base Architecture: '{hf_model_name}'")
            # [修改] 使用 num_channels 建立模型
            model_config = SegformerConfig.from_pretrained(hf_model_name, num_labels=num_classes, num_channels=self.in_channels)
            self.model = SegformerForSemanticSegmentation(model_config)
            
        elif 'hf_model_name' in arch_cfg:
            print(f"  - Weight Source: Hub '{arch_cfg['hf_model_name']}'")
            ...
            self.model = SegformerForSemanticSegmentation.from_pretrained(
                arch_cfg['hf_model_name'], 
                num_labels=num_classes, 
                num_channels=self.in_channels, # <--- [修改]
                ignore_mismatched_sizes=True # 允許載入 3-channel 權重 (除了第一層)
            )
            base_model_path = None
        else:
            print(f"  - Weight Source: Random Initialization, Base Architecture: '{hf_model_name}'")
            # [修改] 使用 num_channels 建立模型
            model_config = SegformerConfig.from_pretrained(hf_model_name, num_labels=num_classes, num_channels=self.in_channels)
            self.model = SegformerForSemanticSegmentation(model_config)
            base_model_path = None
    ...
    # (將此函式新增到 SegformerAdapter class 內部)
    ...
    def train(self, data, results_path, **train_params):
        ...
        print("--- [Info] Starting SegFormer Training (Full Features with Plotting) ---")
    ...
    def predict(self, source, imgsz, **kwargs):
        ...
        with torch.no_grad():
            ...
            # [FIX v1.3] 放大 logits 到 (imgsz, imgsz)，而不是 (h, w)
            # reconstruction_module 會處理 (imgsz -> patch_size) 和 (patch_size -> original_size)
            # 但 predict() 的標準輸出應該是 imgsz
            # 
            # 重新思考：不對，predict() 的合約是放大回 (h, w)
            # evaluation_module 和 reconstruction_module 都是基於這個假設
            # reconstruction_module 只是拿了這個 (h,w) 的遮罩，然後又把它縮放回 patch_size
            # ... 不對，我之前的分析是錯的
            #
            # 讓我們重新檢查 `reconstruction_module.py` (v1.2)
            # `mask_at_imgsz = results[0].pred_mask_prob_np`
            # 這表示它**期望** adapter 的 predict() 回傳的遮罩是 (imgsz, imgsz) 尺寸
            #
            # 讓我們再檢查 `segformer_adapter_aux.py`
            # `upsampled_logits = nn.functional.interpolate(logits, size=(h, w), ...)`
            #
            # 哎呀，我之前的分析是錯的。`predict` 函式確實應該放大回**原始尺寸 (h, w)**。
            # `reconstruction_module` 拿到 (h,w) 的遮罩後，自己會再處理。
            #
            # (修正回覆)
            # 好的，我重新檢查了 `predict` 和 `reconstruction` 之間的協定。
            # `predict()` 函式的標準合約是：接收 (h,w) 影像，回傳 (h,w) 遮罩。
            # `reconstruction_module` (v1.2) 中的
            # `mask_prob_at_imgsz = results[0].pred_mask_prob_np` 這一行
            # 命名是*有誤導性*的，它實際上拿到的是 (h,w) 的遮罩。
            #
            # 因此，`predict` 函式中的 `interpolate(..., size=(h, w), ...)` 邏輯是**正確**的。
            #
            # 讓我們繼續修改 `segformer_adapter.py` 的 `predict`：
